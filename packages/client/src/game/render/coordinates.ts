...
export function tileToWorld(coordinate: Coordinate, gridWidth: number, gridHeight: number): Vector3 {
...
export function worldToTile(x: number, z: number, gridWidth: number, gridHeight: number): Coordinate {
  const offsetX = ((gridWidth - 1) * TILE_SIZE) / 2
  const offsetZ = ((gridHeight - 1) * TILE_SIZE) / 2
  return {
