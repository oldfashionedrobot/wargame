...
| **Elevation is visual only, and capped at 0.25** | Height is a look, never data: a mountain's pad and a bridge's deck raise where a unit *stands*, but the map has no elevation, climbing costs nothing extra, and `screenToTile` still intersects `y = 0` rather than mesh-picking. The cap is what keeps that honest — a surface at height `h` draws `1.25h` tiles off, so a third of a tile passes and half a tile is a misclick | Elevation as a *rule* — cover, movement cost, line of sight — needs the picking question answered properly (mesh picking, or a lookup that knows each tile's height). ⚠️ **But answering it is not enough, which a spike established rather than guessed.** Raising a tile to a full 1.0 and looking at it, the tile draws 1.25 tiles up-screen and visually occupies the tile *behind* it: identity breaks on sight, with picking irrelevant to the problem. The height a board can carry is set by the camera — 38.6° above the horizontal — before it is set by picking, so real elevation wants a camera decision first and a picking rewrite second |
