...
value only appears raw on road or bridge — `infantry → infantry 55` lands as 49
on plains. Worth knowing before reading a printed number as a bug.
...
- **9b** `Unit` gains `health`, and `createMatchState` sets it — ⚠️ *one* place now, since the army left the maps; this used to say "update the starting units" when there were six copies of them. (`unitTypeId` arrived in 6a.) ⚠️ **`types.ts` currently asserts the opposite and has to change with this.**
  The comment on `unitTypeId` reads "movement range today, **health** and combat
  stats later -- lives on the catalog entry this names, not on the instance."
  Health is the one thing on that list that does not: it changes every time
  something is hit. The comment is wrong the moment 9b lands, and it sits
  exactly where 9b lands. **`maxHealth` does not go on `Unit`** — it is static per unit type, which is the exact distinction 6a exists to draw, and putting it on every instance would re-introduce the duplication that moving `movementRange` onto `UnitType` just removed. If every unit tops out at 100 it is a constant in `shared/`; the day one doesn't, it is a `UnitType` field. **No migration** — `Unit` lives inside `GameState`, which is a JSON blob, so the shape changes without the schema moving. That is the JSON-blob decision paying off, and it is why `map_id` in phase 6 is the first migration rather than this.
...
- **9d** ✅ **Shipped.** `range: { min, max }` on `UnitType`: infantry `{1,2}`, cavalry `{1,1}`, artillery `{2,5}`. ⚠️ **Those three settle more than reach.** Infantry at two tiles gets a choice against a battery — trade at range and be answered, or close to one where the gun cannot fire. Cavalry at `{1,1}` is shot at the whole way in, cannot answer anything that outranges it, and is safe from a counter once it arrives: closing is the cost and the charge is the point. Artillery's `min: 2` is its defining weakness and the reason cavalry has a job at all. The catalog test asserts the *contract* — integers, `min ≥ 1`, `max ≥ min` — plus one design assertion, that some unit stays helpless up close, because a gun that can defend itself at arm's length leaves cavalry with nothing to do and nothing else would say so.

  Original note follows. Ahead of the command step rather than after it, so nothing is ever built against an adjacency that is about to stop being true. ⚠️ **No `canMoveAndAttack` and no direct/indirect flag** — everything moves and attacks, and `min: 2` describes a gun rather than classifying it. Two numbers per unit.
...
- **9e** ✅ **Shipped, and half of what it said.** `snapUnits` became `syncUnits`: it removes meshes the authority no longer lists, then positions the rest. ⚠️ **No creation half, because nothing can add a unit** — `applyEvents` only maps over `units`, and there is no production, reinforcement or recruitment; units enter state once in `createMatchState`, before the renderer exists. Writing the add path would have been dead code plus a second creation site to drift from the constructor's. ⚠️ **Unverifiable until 9f**: nothing dies yet, so the removal branch has no way to run. The renderer has no test coverage, so the check is a browser one and it belongs to the step that first kills something.
...
**Build the tuning harness first** — after 9c, which is what it runs. ⚠️ It lives in `packages/shared/scripts/`, run with `bun`, and is the one thing in the repo that imports `shared/` without being `server/` or `client/` — which is only possible *because* `shared/` is pure. ⚠️ **Give it a program**, which costs five lines and was cheaper than expected. `packages/shared/tsconfig.scripts.json` extends the existing config with `types: ["node"]` and `include: ["scripts"]`, plus one entry in the root `references`. Verified: `tsc -b --force` catches a deliberate type error under `scripts/` and exits 0 when clean — **no `composite: true`**, and no new dependency. `types: ["node"]` is required rather than decorative, because `lib: ["ES2023"]` has no `console` and the root's `@types/node` is not auto-included across the isolated workspace install; declaring it on *this* program leaves `src`'s config untouched, so the rulebook gains nothing. ⚠️ **It lands with the first script, not before it** — `include` against an empty directory is `error TS18003: No inputs were found`, exit 2. Specified once, under Tuning. It prints **hits-to-kill**, not raw damage or a distribution; that section argues why, and this one used to say something slightly different, which is how two harnesses get built. This is also the first real use of the purity invariant: `shared/` is pure and rolls are inputs, so the script needs no browser and no server.
