...
**Build the tuning harness first** — after 9c, which is what it runs. ⚠️ It lives in `packages/shared/scripts/`, run with `bun`, and is the one thing in the repo that imports `shared/` without being `server/` or `client/` — which is only possible *because* `shared/` is pure. ⚠️ **Nothing there is typechecked**, and that is a decision to take rather than discover: `shared` has no program of its own, so `tsc -b --force` never sees the directory — the same reason its test files are not checked. Either accept it on the same terms ("verified by running") or give the script a program. Specified once, under Tuning. It prints **hits-to-kill**, not raw damage or a distribution; that section argues why, and this one used to say something slightly different, which is how two harnesses get built. This is also the first real use of the purity invariant: `shared/` is pure and rolls are inputs, so the script needs no browser and no server.
