...
## S6 — ⬜ Every unit moves every turn

**`ACTIONS_PER_TURN` goes back to uncapped**, which is Advance Wars: each unit
acts once a turn, the player picks the order, and the turn ends when the last of
them has gone. The constant stays — it is the dial, and dialling it down is how
a one-command turn gets tried again later — but its **default becomes the model
we are following** rather than a deviation from it.

⚠️ **`null` is the cap, not `Infinity`.** `Math.min(Infinity, roster)` would
pick the roster for free and need no branch at all, which is the tempting
version — but `Infinity` does not survive JSON, where it serialises to `null`
anyway. Ruleset versioning is already on the roadmap, so the day a match records
the rules it was played under, that constant becomes match data and the value
silently changes meaning in transit. Spending one branch now costs a line and
retires the migration:

```ts
export const ACTIONS_PER_TURN: number | null = null; // null: no cap

export function actionsAllowed(state, budget = ACTIONS_PER_TURN): number {
  const roster = ownUnits(state).length;
  return budget === null ? roster : Math.min(budget, roster);
}
```

⚠️ `=== null` rather than `== null`, so an explicitly-passed `undefined` still
falls through to the default instead of quietly meaning *uncapped*. The two
readings differ the moment a caller forwards an optional argument.

**The auto-end stays, and End Turn stays with it.** Advance Wars does not end a
turn on its own, but the reason does not carry: an AW turn also holds production
and CO powers, so "every unit has acted" is not the same as "the turn is over"
there. Here it is exactly the same thing, so ending on the last action costs
nothing and saves a click. End Turn keeps the one job that is genuinely its own
— stopping early, before committing every unit.

### What it touches, and an honest note about why

`turns.ts` is one line. Then five test files, because shipping the constant at
**1** made a single move carry its own turn end, and several fixtures were
rewritten around that:

- `applyEvents.test.ts` — its multi-turn script was made to alternate, one
  command per player. It goes back to several moves a side between turn ends.
- `match.test.ts` — the `since` test loses its explicit `endTurn` and gains it
  back; two event-count assertions go from 2 to 1.
- `http.test.ts` — the same two counts.
- ⚠️ **Worth doing while both files are open: derive `blue-1` and `red-1`'s
  starting squares from `createMatchState(getMap('classic'))` rather than
  writing them down.** They are hardcoded today and have broken on a content
  change twice already; S5 would be the third. Deriving them retires that for
  good and makes S5 free in these two files.
- ⚠️ `action.test.ts` and one comment in `match.test.ts` **still pass while
  saying something false** — they explain themselves in terms of a budget of 1.
  A stale comment that a green suite cannot see is the thing to go looking for
  here rather than to trip over later.

`turns.test.ts` needs nothing at all: it passes an explicit budget in every
assertion, so none of it depends on what the default happens to be. That was the
reason for making the budget an argument, and this is the day it pays. It gains
one case rather than changing any — that `null` means the roster, which is the
only new behaviour here.

⚠️ **The lesson worth keeping.** The churn is self-inflicted: a constant whose
default is a deviation from the model makes every fixture written against it a
deviation too. Default to the thing being modelled, and let the dial be the
experiment.

### What it unblocks

⚠️ **S5 becomes answerable.** Board size and the turn budget were one decision;
settling the budget at the roster size means Advance Wars' own dimensions are a
real reference at last — its boards assume every unit moves every turn, exactly
as ours now will. The rest of S5's argument still holds, and still discounts
those numbers for having no production behind ours.
